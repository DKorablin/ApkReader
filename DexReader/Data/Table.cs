...
namespace AlphaOmega.Debug.Data
{
	...
	public class Table : ITable
	{
		...
		public Row[] Rows { get { return this._rows.ToArray(); } }
		IRow[] ITable.Rows { get { return this._rows.ToArray(); } }
		...
		internal void ReadSection(DexFile file, UInt32 offset, UInt32 maxItems)
		{
			UInt32 rowIndex = 0;
			...
			while(rowIndex < maxItems)
			{
				...
				for(UInt32 loop = 0; loop < cells.Length; loop++)
				{
					...
					cells[loop] = this.ReadCellPayload(file, column, cells, ref offset);
				}
		...
		protected internal virtual Cell ReadCellPayload(DexFile file, Column column, Cell[] cells, ref UInt32 offset)
		{
